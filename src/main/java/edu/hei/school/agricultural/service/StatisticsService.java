...
public class StatisticsService {
    ...
    public List<CollectivityLocalStatistics> getLocalStats(
        ...
        List<Map<String, Object>> earned    = repository.getEarnedAmountByMember(collectivityId, from, to);
        List<Map<String, Object>> unpaid    = repository.getUnpaidAmountByMember(collectivityId, to);
        List<Map<String, Object>> assiduity = repository.getMemberAssiduityByCollectivity(collectivityId, from, to);

        Map<String, Double> unpaidMap = new HashMap<>();
        for (Map<String, Object> u : unpaid) {
            unpaidMap.put((String) u.get("id"), ((Number) u.get("unpaid_amount")).doubleValue());
        }

        Map<String, Double> assiduityMap = new HashMap<>();
        for (Map<String, Object> a : assiduity) {
            assiduityMap.put((String) a.get("id"), ((Number) a.get("assiduity_percentage")).doubleValue());
        }
        ...
        for (Map<String, Object> e : earned) {
            ...
            result.add(mapper.toLocalStats(
                    ...
                    assiduityMap.getOrDefault(memberId, 0.0)
            ));
        }
    ...
    public List<CollectivityOverallStatistics> getGlobalStats(
        ...
        List<Map<String, Object>> percentage = repository.getCurrentMembersPercentage();
        List<Map<String, Object>> newMembers = repository.getNewMembers(from, to);
        List<Map<String, Object>> assiduity  = repository.getOverallAssiduityByCollectivity(from, to);

        Map<String, Integer> newMembersMap = new HashMap<>();
        for (Map<String, Object> n : newMembers) {
            newMembersMap.put((String) n.get("collectivity_id"), ((Number) n.get("new_members")).intValue());
        }

        Map<String, Double> assiduityMap = new HashMap<>();
        for (Map<String, Object> a : assiduity) {
            assiduityMap.put((String) a.get("collectivity_id"), ((Number) a.get("assiduity_percentage")).doubleValue());
        }
        ...
        for (Map<String, Object> p : percentage) {
            ...
            result.add(mapper.toGlobalStats(
                    ...
                    assiduityMap.getOrDefault(collectivityId, 0.0)
            ));
        }
